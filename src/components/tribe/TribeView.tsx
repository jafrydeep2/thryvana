...
const TribeView = ({ isLoading: initialLoading = false }: TribeViewProps) => {
  ...
  const [selectedTribeID, setTribeId] = useState(null)
  ...
  const getTribeMembersFromSupabase = async (tribeId: string): Promise<{ id: string; name: string; avatar: string; tribeId: string }[]> => {
    ...
    try {
      const { data, error } = await supabase
        .from("check_ins")
        .select("user_id, tribe_id, users(id, username, email)")
        .eq("tribe_id", tribeId)
        .limit(1000)
      ...
      // Use a Set to track unique user_ids
      const uniqueIds = new Set();
      const distinctUsers = data?.filter((item: any) => {
        if (!uniqueIds.has(item.user_id)) {
      ...
      return distinctUsers?.map((item: any) => ({
        id: item.user_id,
  ...
  //       const membersPromises = tribeIds.map(tribeId => getTribeMembersFromSupabase(tribeId));
  ...
  const getTribeCheckIns = async () => {
    try {
      ...
      // Get all check-ins ordered by time
      const { data, error } = await supabase
        .from("check_ins")
        .select("*, users(username, email), reactions(*)")
        .order("check_in_time", { ascending: false });
      ...
      if (data.length > 0) {
        ...
        // Create an array to hold all tribe members
        const allMembers = [];
        
        // For each tribe, get its members directly
        for (const tribeId of tribeIds) {
          // Get unique users who have checked in to this tribe
          const { data: tribeCheckIns, error: tribeError } = await supabase
            .from("check_ins")
            .select("user_id, tribe_id, users(user_id, username)")
            .eq("tribe_id", tribeId)
            .order('check_in_time', { ascending: false });
          ...
          // Track unique users
          const uniqueIds = new Set();
          const distinctUsers: any = tribeCheckIns?.filter((item) => {
            if (!uniqueIds.has(item.user_id)) {
              uniqueIds.add(item.user_id);
              return true;
            }
          ...
          const tribeMembers = distinctUsers.map((item) => ({
            id: item.user_id,
            name: item.users?.username || "Unknown",
            avatar: item.users?.avatar_url || "",
            tribeId: item.tribe_id
          ...
          // Add these members to the overall list
          allMembers.push(...tribeMembers);
        }
        
        setAllTribeMembers(allMembers);
        
        // Filter members by frequency if needed
        if (frequencyFilter) {
          const filteredMembers = allMembers.filter(member => 
            tribeFrequencies[member.tribeId] === frequencyFilter
          );
          ...
          // If no frequency filter, use all members
          setTribeMembers(allMembers);
        }
      }
  
      // Process check-ins with additional properties
      const enhancedCheckIns = data.map((checkIn) => ({
        ...checkIn,
        username: checkIn?.users?.username || "Unknown",
        lastActive: checkIn.check_in_time,
        createdAt: checkIn.check_in_time,
        media: checkIn.media_url || [], // Ensure media is always an array
        content: checkIn.content || 'No content available',
        photo: checkIn.photo_url || '',
        reactions: checkIn.reactions || [],
        tribeFrequency: tribeFrequencies[checkIn.tribe_id] || "daily"
      }));
  
      setAllCheckIns(enhancedCheckIns);
  
      // Apply frequency filter to check-ins if needed
      if (frequencyFilter) {
        const filteredCheckIns = enhancedCheckIns.filter(checkIn => 
          checkIn.tribeFrequency === frequencyFilter
        );
        ...
        // If no frequency filter, use all check-ins
  ...
  const handleFrequencyChange = (frequency: "daily" | "weekly" | "monthly" | any) => {
  ...
  useEffect(() => {
    if (activeGoal?.timeframe) {
      handleFrequencyChange(activeGoal?.timeframe?.toLowerCase())
    }
  }, [activeGoal?.timeframe, allTribeMembers, allCheckIns])
